...
var Capturer = function () {
	...
	_createClass(Capturer, [{
		...
		key: '_getCanvasDataUrl',
		value: function _getCanvasDataUrl(canvasElm) {
			var canvasDataUrl = '';
			try {
		...
		value: function _recursiveWalk(domElm, newElm, handleCss) {
			...
			if (!this._isHead && domElm.tagName.toLowerCase() === 'canvas') {
				var canvasDataUrl = this._getCanvasDataUrl(domElm);
				if (canvasDataUrl) {
		...
		key: '_handleElmValue',
		value: function _handleElmValue(domElm, newElm) {
